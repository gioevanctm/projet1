...
from tkinter import *
def action():
    path = askopenfilename(title="Ouverture fichier excel",
    ...
    for i in range(1, sheet.nrows):
        ...
        w.record(Telephone=sheet.row_values(i)[0], Departement=sheet.row_values(i)[1],
                 code_commune_insee=sheet.row_values(i)[2],
                 voie=sheet.row_values(i)[3], code_postal=sheet.row_values(i)[4], Commune=sheet.row_values(i)[5],
                 geocodage_epsg=sheet.row_values(i)[6],
...
ok = Button(Fenetre, text="conversion excel en shapefile", command=action)
